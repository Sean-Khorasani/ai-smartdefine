...
function createWordCard(word) {
  ...
  if (word.id) flashcardBtn.setAttribute('data-id', word.id);
  if (word.provider) flashcardBtn.setAttribute('data-provider', word.provider);
  ...
  // Add provider source badge
  const providerSpan = document.createElement('span');
  providerSpan.className = 'provider-badge';
  
  // Determine provider display based on the actual provider value
  if (!word.provider || word.provider === 'Unknown') {
    providerSpan.textContent = '❓ Unknown';
    providerSpan.style.backgroundColor = '#9e9e9e';
  } else if (word.provider === 'FreeDictionary' || word.provider === 'text') {
    providerSpan.textContent = '📚 Dictionary';
    providerSpan.style.backgroundColor = '#2196F3';
  } else {
    // This is an LLM provider
    providerSpan.textContent = `🤖 ${word.provider}`;
    providerSpan.style.backgroundColor = '#4CAF50';
  }
  
  providerSpan.style.color = 'white';
  providerSpan.style.padding = '2px 6px';
  providerSpan.style.borderRadius = '4px';
...
function handleWordAction(event) {
  ...
  switch (action) {
    case 'view':
      viewWord(word, category, wordId, provider);
      break;
    case 'export':
      exportWord(word, category, wordId, provider);
      break;
    case 'flashcard':
      showFlashcard(word, category, wordId, provider);
      break;
    case 'delete':
...
// Parse dictionary response for view - similar to content script logic
async function parseDictionaryWordFormResponseForView(responseText, originalWord) {
  try {
    if (!responseText || typeof responseText !== 'string') {
      return null;
    }
    
    const base = originalWord.toLowerCase();
    let type = 'unknown';
    let form = 'base form';
    ...
    // Generate forms based on the detected type
    if (type !== 'unknown') {
      const forms = await generateFormsForTypeInView(base, type);
      return {
        base: base,
        form: 'base form',
        type: type,
    ...
    console.log('Error parsing dictionary word form response for view:', error);
    return null;
  }
}

// Generate word forms for view - simplified version
async function generateFormsForTypeInView(word, type) {
  const forms = [];
  
  switch (type) {
    case 'noun':
      // Generate basic noun forms
      let plural = word + 's';
      ...
      forms.push(
        { form: 'singular', word: word, example: `a ${word}` },
        { form: 'plural', word: plural, example: `multiple ${plural}` },
        { form: 'possessive', word: possessive, example: `the ${possessive} value` }
      );
      break;
    ...
    case 'verb':
      ...
      let past = word + 'ed';
      let presentParticiple = word + 'ing';
      let pastParticiple = word + 'ed';
      
      // Handle common irregular patterns
      if (word.endsWith('e')) {
        past = word.slice(0, -1) + 'ed';
        presentParticiple = word.slice(0, -1) + 'ing';
        pastParticiple = word.slice(0, -1) + 'ed';
      } else if (word.endsWith('y') && word.length > 1 && !'aeiou'.includes(word[word.length-2])) {
      ...
      forms.push(
        { form: 'infinitive', word: word, example: `to ${word}` },
        { form: 'past tense', word: past, example: `they ${past}` },
        { form: 'present participle', word: presentParticiple, example: `they are ${presentParticiple}` },
        { form: 'past participle', word: pastParticiple, example: `they have ${pastParticiple}` }
      );
      break;
      
    case 'noun and verb':
      // Generate both noun and verb forms
      let nounPlural = word + 's';
      if (word.endsWith('s') || word.endsWith('x') || word.endsWith('z') || 
          word.endsWith('ch') || word.endsWith('sh')) {
        nounPlural = word + 'es';
      } else if (word.endsWith('y') && word.length > 1 && !'aeiou'.includes(word[word.length-2])) {
        nounPlural = word.slice(0, -1) + 'ies';
      }
      
      let verbPast = word + 'ed';
      let verbPresent = word + 'ing';
      let verbPastPart = word + 'ed';
      
      if (word.endsWith('e')) {
        verbPast = word.slice(0, -1) + 'ed';
        verbPresent = word.slice(0, -1) + 'ing';
        verbPastPart = word.slice(0, -1) + 'ed';
      }
      
      forms.push(
        { form: 'singular', word: word, example: `a ${word}` },
        { form: 'plural', word: nounPlural, example: `multiple ${nounPlural}` },
        { form: 'infinitive', word: word, example: `to ${word}` },
        { form: 'past tense', word: verbPast, example: `they ${verbPast}` },
        { form: 'present participle', word: verbPresent, example: `they are ${verbPresent}` },
        { form: 'past participle', word: verbPastPart, example: `they have ${verbPastPart}` }
      );
      break;
      
    case 'adjective':
      // Generate basic adjective forms
      let comparative = word + 'er';
      let superlative = word + 'est';
      ...
      forms.push(
        { form: 'positive', word: word, example: `a ${word} thing` },
        { form: 'comparative', word: comparative, example: `${comparative} than others` },
        { form: 'superlative', word: superlative, example: `the ${superlative} of all` }
      );
      break;
      
    default:
      forms.push({ form: 'base', word: word, example: `this is ${word}` });
      break;
  }
...
function showFlashcard(word, category, wordId = null, provider = null) {
  let wordData;
  
  if (wordId && provider) {
    // Find specific word by ID and provider
    wordData = wordLists[category]?.find(w => w.id === wordId && w.provider === provider);
  } else {
...
window.viewWord = async function(word, category, wordId = null, provider = null) {
  // Find the word data using proper lookup logic
  let wordData = null;
  ...
  if (!wordData && provider) {
    wordData = wordLists[category]?.find(w => w.word === word && w.provider === provider);
  }
  if (!wordData) {
    // Final fallback to word-only matching
    wordData = wordLists[category]?.find(w => w.word === word);
  }
  
  if (!wordData) {
    console.error('Word not found in viewWord function');
    return;
  }
  ...
  // Create explanation section - reparse with new word type detection
  const explanationDiv = document.createElement('div');
  explanationDiv.style.cssText = 'line-height: 1.8; color: #333; font-size: 15px;';
  
  // Parse word data to extract proper word type and create Word Analysis section
  let wordInfo = null;
  if (wordData.explanation && typeof wordData.explanation === 'string' &&
      !wordData.explanation.includes('Unable to get definition') &&
      !wordData.explanation.includes('This word was not found in the dictionary')) {
    try {
      // Try to parse using the updated dictionary parsing logic
      if (wordData.explanation.includes('**Word Type:**')) {
        // Import the parsing function from content script context
        wordInfo = await parseDictionaryWordFormResponseForView(wordData.explanation, wordData.word);
      }
    } catch (error) {
      console.log('Failed to parse word info for view:', error);
    }
  }
  
  // Add Word Analysis section if we have word info
  if (wordInfo && wordInfo.type && wordInfo.forms) {
    ...
    analysisTitle.style.cssText = `
      ...
      gap: 8px;
    `;
    analysisTitle.innerHTML = '<span>📝</span><span>Word Analysis</span>';
    
    const wordTypeDiv = document.createElement('div');
    wordTypeDiv.style.cssText = 'margin-bottom: 12px;';
    wordTypeDiv.innerHTML = `<strong style="color: #4CAF50;">Word Type:</strong> <span style="color: #2c3e50;">${wordInfo.type}</span><br><strong style="color: #4CAF50;">Current Form:</strong> <span style="color: #2c3e50;">${wordInfo.form}</span>`;
    
    if (wordInfo.forms && wordInfo.forms.length > 0) {
      const formsDiv = document.createElement('div');
      formsDiv.style.cssText = 'margin-top: 12px;';
      
      const formsTitle = document.createElement('div');
      formsTitle.style.cssText = 'font-weight: 600; color: #4CAF50; margin-bottom: 8px;';
      formsTitle.textContent = 'Other Forms:';
      
      const formsList = document.createElement('div');
      formsList.style.cssText = 'margin-left: 16px;';
      
      wordInfo.forms.forEach(formData => {
        const formItem = document.createElement('div');
        formItem.style.cssText = 'margin-bottom: 4px; color: #2c3e50;';
        formItem.innerHTML = `<strong>${formData.word}</strong> (${formData.form}) - <em style="color: #666;">${formData.example}</em>`;
      ...
      analysisDiv.appendChild(wordTypeDiv);
      analysisDiv.appendChild(formsDiv);
    } else {
      analysisDiv.appendChild(analysisTitle);
      analysisDiv.appendChild(wordTypeDiv);
    }
    
    explanationDiv.appendChild(analysisDiv);
  }
  
  // Format the main explanation content
...
window.exportWord = function(word, category, wordId = null, provider = null) {
  let wordData;
