...
// Generate verb forms (base, past, participles) with API validation
async function generateVerbForms(word) {
  const base = word.toLowerCase().trim();
  let past, thirdPerson, presentParticiple, pastParticiple;
  
  // Generate third person singular
  ...
  if (base.endsWith('e')) {
    ...
    pastParticiple = base.slice(0, -1) + 'ied';
             ...
             !base.endsWith('er') && !base.endsWith('nt')) {
    // Double consonant before adding -ed (but not for words ending in 'er' or 'nt')
    past = base + base[base.length-1] + 'ed';
    pastParticiple = base + base[base.length-1] + 'ed';
  } else {
    past = base + 'ed';
  ...
  if (base.endsWith('e') && !base.endsWith('ee') && !base.endsWith('oe') && !base.endsWith('ye')) {
    presentParticiple = base.slice(0, -1) + 'ing';
             ...
             'aeiou'.includes(base[base.length-2]) && !'aeiou'.includes(base[base.length-3]) &&
             !base.endsWith('er') && !base.endsWith('nt')) {
    // Double consonant before adding -ing (but not for words ending in 'er' or 'nt')
  ...
  try {
    ...
    const [isThirdPersonValid, isPastValid, isPresentParticipleValid, isPastParticipleValid] = await Promise.all([
      isActualEnglishWord(thirdPerson),
      isActualEnglishWord(past),
      isActualEnglishWord(presentParticiple),
      isActualEnglishWord(pastParticiple)
    ]);
    ...
    if (!isPastValid) {
      past = base + 'ed';
      pastParticiple = base + 'ed'; // Past participle usually same as past tense for regular verbs
    }
    if (!isPresentParticipleValid) {
      presentParticiple = base + 'ing';
    }
    if (!isPastParticipleValid && isPastValid) {
      pastParticiple = past; // Use validated past tense
    } else if (!isPastParticipleValid) {
      pastParticiple = base + 'ed';
    }
...
// Enhanced word form detection with API validation
async function getWordFormInfoWithAPI(word, settings = null) {
  // Start with rule-based detection as fallback
  const ruleBasedInfo = getWordFormInfo(word);
  
  try {
    // Try to get more accurate information from API
    if (settings) {
      const hasAPIKey = Object.values(settings.providers || {}).some(p => p.enabled && p.apiKey && p.apiKey.trim().length > 0);
      
      if (hasAPIKey) {
        // Create a morphological analysis prompt
        const morphPrompt = `Analyze the word "${word}" and provide ONLY this exact format:

BASE_FORM: [the dictionary/lemma form]
WORD_TYPE: [noun|verb|adjective|adverb|preposition|etc.]
...
Be accurate and concise. If uncertain, provide your best analysis.`;
        ...
        try {
          ...
          const apiInfo = parseAPIWordFormResponse(apiText, word);
          
          if (apiInfo && apiInfo.base && apiInfo.base !== 'unknown') {
            return apiInfo;
          }
        } catch (error) {
          console.log('LLM API failed for morphological analysis, trying dictionary...');
        }
      }
      
      // Fallback to dictionary API for morphological info
      try {
        const dictResponse = await browser.runtime.sendMessage({
          command: 'callFreeDictionaryAPI',
          text: word
        });
        
        const dictInfo = parseDictionaryWordFormResponse(dictResponse, word);
        if (dictInfo && dictInfo.base && dictInfo.base !== 'unknown') {
          return dictInfo;
        }
      } catch (error) {
        console.log('Dictionary API also failed for morphological analysis');
      }
    }
  } catch (error) {
    console.log('API validation failed, using rule-based approach:', error);
  }
...
async function generateFormsForType(word, type) {
  ...
  switch (type) {
    ...
    case 'verb':
      ...
      const verbForms = await generateVerbForms(word);
      forms.push(
    ...
    case 'noun and verb':
      ...
      const verbForms2 = await generateVerbForms(word);
      forms.push(
    ...
    case 'verb and adjective':
      ...
      const verbForms4 = await generateVerbForms(word);
...
async function generateAdjectiveForms(word) {
  ...
  try {
    ...
    const [isComparativeValid, isSuperlativeValid] = await Promise.all([
      isActualEnglishWord(comparative),
      isActualEnglishWord(superlative)
    ]);
...
async function createResponseModal(selectedText, response, context = null, provider = null) {
  ...
  // Word type and form information from main dictionary response
  let wordInfo;
  ...
  if (response !== "Loading explanation..." && 
    ...
    try {
      // Extract word type from the main dictionary response
      if (response && typeof response === 'string' && response.includes('**Word Type:**')) {
        wordInfo = await parseDictionaryWordFormResponse(response, selectedText);
      }
      
      // Fallback to API-based analysis if dictionary parsing failed
      if (!wordInfo) {
      ...
      console.log('API validation failed, using rule-based word info:', error);
...
async function showSaveToListModal(word, explanation, context = null, provider = null, wordInfo = null) {
  ...
  document.getElementById('confirm-save').onclick = async () => {
    ...
    await saveWordToList(word, explanation, selectedCategory, notesInput.value.trim(), context, provider, wordInfo);
    saveModal.remove();
...
// Save word to storage with API-validated base form
async function saveWordToList(word, explanation, category, notes, context = null, provider = null, wordInfo = null) {
  const storage = await browser.storage.local.get(['wordLists', 'selectedProvider', 'prompt', 'providers']);
  const wordLists = storage.wordLists || {};
  ...
  // Use provided word form info if available, otherwise analyze the word
  let formInfo;
  if (wordInfo) {
    console.log('Using provided word info for saving (from main modal)');
    formInfo = wordInfo;
  } else {
    console.log('No word info provided, analyzing word for save');
    try {
      const settings = { selectedProvider: storage.selectedProvider, prompt: storage.prompt, providers: storage.providers };
      formInfo = await getWordFormInfoWithAPI(word, settings);
    } catch (error) {
      console.log('API validation failed during save, using rule-based approach:', error);
      formInfo = getWordFormInfo(word);
    }
  }
  ...
  const wordData = {
    ...
    notes: notes,
    context: context,
    wordType: formInfo.type, // Store validated word type
    currentForm: formInfo.form, // Store current form info
    otherForms: formInfo.forms, // Store other forms
